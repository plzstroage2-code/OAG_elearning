...
โปรเจกต์ที่ใช้:
- Supabase: https://supabase.com/dashboard/project/sgjianadimmgvxkiixrm
- GitHub: https://github.com/techapp-JN/OAGTH_Lucky_award
...
```env
NEXT_PUBLIC_LUCKYDRAW_SUPABASE_URL=https://sgjianadimmgvxkiixrm.supabase.co
...
???????????????????????????? root ??? repository ????
???????? draw ??? origin ??? branch `production-readiness` ???????? `origin/main` ??? commit `90ef85f`
???????????????????? Git ?????? force-push ????????? merge main ?????????????????????/?????/environment ?????

??????????? e-Learning Draw ??? `draw_private.events` ??? `stage_signals` ???????? schema ?????????
migration ???????????????????????????????????? ???????????????????????????
???????????????/????????????? ????????????????????????
...
???? review ???? commit/push branch ??? ???? PR ?? main ????? CI ????
???? merge main ????? Supabase ??? 1?3 ??? Vercel ??? 5 ?????? ????? main ??? trigger production deployment ?????????
workflow `.github/workflows/ci.yml` ?????????? build ????????? credentials ???? ?????? deploy artifact ????
Vercel ???? build ???????? Production/Preview environment ?????????

## 5. Vercel

1. เปิดโปรเจกต์ `oag4/oagth-newgen-lucky`
2. Settings → Git: ตรวจว่าเชื่อม `techapp-JN/OAGTH_Lucky_award` และ Production Branch ถูกต้อง
